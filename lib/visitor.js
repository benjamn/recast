...
var visit = exports.visit = function(node, visitors) {
   ...
   * called.
   */
  function visitNode(path, visitor, done, currVisitors) {
    var thisObj = {};
    if (!visitor) {
      return done();
    }
    ...
    if (nextVisitor) {
      thisObj.traverse = function(newVisitors) {
        ...
        return visitNode(path, nextVisitor, done, currVisitors);
      }
    } else if (traversalState.toVisit.length === 0) {
      thisObj.traverse = function(newVisitors){
        done(newVisitors);
  ...
  function traverseNode(path) {
    ...
    var doneFunc = function() {
      if (!nodeTraversed) {
        ...
        assert(false, "Attempting to traverse a node that has already been" +
               " traversed.");
      }
    };
    ...
    if (Node.check(value)) {
      ...
      visitNode(path, getNextVisitor(path), doneFunc, genVisitors(path));
      if (!nodeTraversed) {
        assert(false, "nodePath.traverse called more than once!");
      }
    }
