// TODO Move this into its own node package?
// TODONE Check semicolons, especially where there are no fields (so .build
//        is last).]
// TODONE Check MISSING semicolons.
// TODONE Stringify builder parameter names.
// TODONE Check builder parameter names.
// TODONE Check types that should be buildable but aren't listed in API.
// TODONE Make field order match builder parameter order, where possible.
// TODONE Check for fields that should have defaults because they might not
//        be listed as builder parameters.
// TODONE Figure out how to generate the builder function name.
// TODONE Make the isBlah functions behave as "proper" types.
// TODONE Use this file to generate builder functions.
// TODONT "Seal" types before exporting.
//        (Just return the type instead of the Def object.)
// TODO Write tests.
// TODO Support meaningful naming of types.

// THOUGHTS
//
// The ability never to stay stuck so long that you lose your motivation
// is all you need. Other combinations of virtues might also be all you
// need, but those two are sufficient.
//
// I am at my most productive when I am steadily generating a list of
// TODOs that I know how to address but can address later. When the stream
// of TODOs runs out, I'm done.
...
Type.fromObject = function(obj) {
    ...
    return new Type(function(value) {
        return isObject.check(value) && fields.every(function(field) {
            return field.type.check(value[field.name]);
        });
    }, function() {
        return "{ " + fields.join(", ") + " }";
    });
    ...
    Object.defineProperties(self, {
        ...
        type: {
            value: new Type(function(value) {
                ...
                // We rely entirely on self.typeName and value.type to
                // make subtyping determinations. Interface conformance is
                // checked only when a new value is created using a
                // builder function.
