...
Lp.sliceString = function(start, end, options) {
    ...
    for (var line = start.line; line <= end.line; ++line) {
        ...
        if (line === end.line) {
            var ec = Math.min(lineLength, end.column);
            if (ec < indent) {
                indent = ec;
                sliceEnd = sliceStart;
            } else {
                sliceEnd -= lineLength - ec;
            }
        ...
        if (line === start.line) {
            var sc = Math.max(start.column, 0);
            lineLength -= sc;
        ...
        var before = info.line.slice(0, sliceStart);
        if (options.reuseWhitespace &&
            isOnlyWhitespace(before) &&
            countSpaces(before, options.tabWidth) === indent) {
            // Reuse original spaces if the indentation is correct.
            parts.push(info.line.slice(0, sliceEnd));
            continue;
        }
        ...
        result += info.line.slice(sliceStart, sliceEnd);
