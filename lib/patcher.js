...
exports.getReprinter = function(path) {
    ...
    return function(print) {
        ...
        reprints.forEach(function(reprint) {
            ...
            if (needsLeadingSpace(lines, oldNode.loc)) {
                newLines = linesModule.concat([" ", newLines]);
            }
...
// return"asdf" with e.g. a literal null expression, we run the risk of
// ending up with returnnull, so we need to add an extra leading space in
// situations where that might happen. See #170.
var riskyAdjoiningCharExp = /[0-9a-z_$]/i;
function needsLeadingSpace(lines, loc) {
    var pos = util.copyPos(loc.start);
    return lines.prevPos(pos) &&
