...
function findReprints(newPath, reprints) {
    ...
    Node.assert(newNode.original);
    assert.deepEqual(reprints, []);

    var oldPath = new NodePath(newNode.original);
...
function findObjectReprints(newPath, oldPath, reprints) {
    ...
    var childReprints = [];
    var canReprintChildren = findChildReprints(newPath, oldPath, childReprints);
...
function hasParens(node) {
    ...
    return hasOpeningParen(node) && hasClosingParen(node);
}
...
function findChildReprints(newPath, oldPath, reprints) {
    ...
    if (newPath.needsParens() && !hasParens(oldNode))
        return false;
