...
function findObjectReprints(a, b, reprints) {
    ...
    if (a.type in Syntax) {
        ...
        // If b needs parentheses, then we really want to make sure the
        // original source has parentheses.

        // Do we always have access to source here? Technically we don't
        // care about missing .originals as long as the nodes are
        // deepEquivalent. But we do know that a came from the original
        // tree, so it should always have .loc.lines, right?

        // We can test hasParens against path.needsParens pretty
        // effectively.

        // If operators change, we'll pick up on that anyway.
