...
function skipChars(
    ...
    if (targetComparison < 0) {
        ...
        while (comparePos(targetCursor, targetToPos) < 0 &&
            ...
            if (targetLines.skipSpaces(targetCursor, false, true)) {
                assert.strictEqual(
                    sourceLines.skipSpaces(sourceCursor, false, true),
                    sourceCursor
                );
        ...
        while (comparePos(targetToPos, targetCursor) < 0 &&
            ...
            if (targetLines.skipSpaces(targetCursor, true, true)) {
                assert.strictEqual(
                    sourceLines.skipSpaces(sourceCursor, true, true),
                    sourceCursor
                );
