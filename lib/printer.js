...
function genericPrintNoParens(n, options, print) {
    ...
    case "ModuleDeclaration":
        ...
        if (n.source) {
            assert.ok(!n.body);
            parts.push("from", print(n.source));
        } else {
            parts.push(print(n.body));
        }
    ...
    case "ExportSpecifier":
        var parts = [print(n.id)];

        if (n.name)
            parts.push(" as ", print(n.name));
    ...
    case "ExportDeclaration":
        ...
        if (n["default"]) {
            ...
            if (n.specifiers.length === 1 &&
                ...
                parts.push(
                    " { ",
                    fromString(", ").join(n.specifiers.map(print)),
                    " }"
                );
            }
        ...
        parts.push(" ", print(n.declaration), ";");
    ...
    case "ImportDeclaration":
        ...
        if (!(n.specifiers &&
              n.specifiers.length > 0)) {
            parts.push(" ", print(n.source));

        } else if (n.kind === "default") {
            parts.push(
                " ",
                print(n.specifiers[0]),
                " from ",
                print(n.source)
            );
    ...
    case "VariableDeclaration":
        var parts = [n.kind, " "];
        var maxLen = 0;
        var printed = n.declarations.map(function(decl) {
            var lines = print(decl);
