...
function genericPrintNoParens(path, options, print) {
    ...
    case "FunctionExpression":
        ...
        parts.push(
            "(",
            maybeWrapParams(path, options, print),
            ") ",
            print(path.get("body")));
...
function printMethod(kind, keyPath, valuePath, options, print) {
    ...
    parts.push(
        print(keyPath),
        "(",
        maybeWrapParams(valuePath, options, print),
        ") ",
...
function maybeWrapParams(path, options, print) {
    ...
    var printed = path.get("params").map(print);
    ...
    if (fun.defaults) {
      for (var i = 0; i < defaults.value.length; i++) {
        var def = defaults.get(i);
        if (def.value) {
          printed[i] = concat([printed[i], '=',  print(def)]);
        }
      }
    }
